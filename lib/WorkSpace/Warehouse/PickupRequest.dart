...
class _PickupRequestPageState extends State<PickupRequestPage> {
  ...
  final TextEditingController _useDateController = TextEditingController();
  ...
  Future<void> createWarehouseRequest() async {
    ...
    final itemRequests = _items.map((item) {
      return {
        "item": {"itemId": item['id']},
    ...
    final requestBody = {
      ...
      "activityName": _eventNameController.text.trim(),
      "activityDate": _eventDateController.text.trim(),
      "requesterName": _applicantNameController.text.trim(),
      "pickupDate": _pickupDateController.text.trim(),
      "usageDate": _useDateController.text.trim(),
        ...
        showDialog(
          ...
          builder: (BuildContext context) {
            return AlertDialog(
              ...
              content: Text('取货申请提交成功', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20),),
              actions: <Widget>[
                TextButton(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('取货申请表', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold),), backgroundColor: Color.fromRGBO(29,32,136,1.0),),
            ...
            child: Column(
              children: <Widget>[
                ...
                Row(
                  children: [
                    Expanded(
                      child: _buildDateField(context, '申领日期', _pickupDateController),
                    ),
                    SizedBox(width: 10),
                    Expanded(
                      child: _buildDateField(context, '使用日期', _useDateController),
                    ),
                  ],
                ),
                ...
                ElevatedButton(
                  onPressed: () {
                    ...
                    print('Applicant Name: "${_applicantNameController.text}"');
                    print('Items: $_items');
